...
<schema xmlns="http://www.ascc.net/xml/schematron" xmlns:cda="urn:hl7-org:v3">
    ...
    <phase id='errors'>
        ...
        <active pattern="Telephone_Numbers" />
        <active pattern="ClinicalDocument_languageCode" />
...
serviceProviderOrganization elements shall have name,
    ...
    <pattern id='ClinicalDocument_id' name='ClinicalDocument_id'
    see='&baseURI;#ClinicalDocument_id'>
<!--

Removed restriction on length of UUID (a UUID has 36 characters?)...
...
-->
        <rule id='cda-id-oid' context="/cda:ClinicalDocument/cda:id[contains(@root, '.')]" >
            ...
            <assert diagnostics='L1-12 L1-14' test="not(contains(@root,'..'))">
                Error: A properly formatted OID should not contain two . characters without any 
                intervening digits
            </assert>
    ...
    <pattern id='documentationOf' name='documentationOf' see='&baseURI;#documentationOf'>
        ...
        <rule id='cda-documentationOf' context='cda:ClinicalDocument'>
            <assert diagnostics="L1-51" test='count(cda:documentationOf) = 1'>
                Error: Exactly one ClinicalDocument/documentationOf element shall be present.
            </assert>
        </rule>
    ...
    <pattern name="missedWarnings" id="missedWarnings">
        <rule 
            context='cda:patient/cda:birthTime | 
        ...
        <rule context="/cda:ClinicalDocument/cda:id[contains(@root, '-')]">
            <assert diagnostics='L1-12 L1-13' 
                test="translate(substring(@root, 1, 8),'ABCDEFabcdef0123456789','') = ''">
                Warning: The first four data bytes of the UUID should be represented using hexidecimal 
                digits ([A-Fa-f0-9]).
            </assert>
            <assert diagnostics='L1-12 L1-13' 
                test="translate(substring(@root, 10, 4),'ABCDEFabcdef0123456789','') = ''" >
                Warning: The fifth and sixth data bytes of the UUID should be represented using hexidecimal 
                digits ([A-Fa-f0-9]).
            </assert>
            <assert diagnostics='L1-12 L1-13' 
                test="translate(substring(@root, 15, 4),'ABCDEFabcdef0123456789','') = ''">
                Warning: The seventh and eighth data bytes of the UUID should be represented using hexidecimal 
                digits ([A-Fa-f0-9]).
            </assert>
            <assert diagnostics='L1-12 L1-13' 
                test="translate(substring(@root, 20, 4),'ABCDEFabcdef0123456789','') = ''">
                Warning: The ninth and tenth data bytes of the UUID should be represented using hexidecimal 
                digits ([A-Fa-f0-9]).
            </assert>
            <assert diagnostics='L1-12 L1-13' 
                test="translate(substring(@root, 25, 12),'ABCDEFabcdef0123456789','') = ''">
                Warning: The eleventh through sixteenth data bytes of the UUID should be represented using 
                hexidecimal digits ([A-Fa-f0-9]).
            </assert>
            <assert diagnostics='L1-12 L1-13' test="substring(@root, 9, 1) = '-'">
                Warning: A hyphen should separate the first four data bytes from the remainder of the UUID.
            </assert>
            <assert diagnostics='L1-12 L1-13' test="substring(@root, 14, 1) = '-'">
                Warning: A hyphen should separate the fifth and sixth data byte from the remainder of the UUID.
            </assert>
            <assert diagnostics='L1-12 L1-13' test="substring(@root, 19, 1) = '-'">
                Warning: A hyphen should separate the seventh and eighth data byte from the remainder of the UUID.
            </assert>
            <assert diagnostics='L1-12 L1-13' test="substring(@root, 24, 1) = '-'">
                Warning: A hyphen should separate the ninth and tenth data byte from the remainder of the UUID.
            </assert>
        </rule>
